...
local FOOTSTEP_VOLUME_MULTIPLIER = 2.3
local FOOTSTEP_MAX_DISTANCE = 125
...
function CharacterRealism:MountMaterialSounds(humanoid)
	...
	Util:PromiseChild(rootPart, "Running", function (running)
		...
		local function onStateChanged(old, new)			
			if new.Name:find("Running") then
				while humanoid:GetState() == new do
					...
					local volume = ((speed - 4) / 12) * scale * FOOTSTEP_VOLUME_MULTIPLIER
		...
		running.MaxDistance = FOOTSTEP_MAX_DISTANCE
...
if script:IsA("ModuleScript") then
	...
	-- Sanity check
	--assert(script.Parent:IsDescendantOf(game:GetService("StarterPlayer").StarterPlayerScripts), "RealismClient must be parented to the StarterPlayerScripts!")
